...
public class UserUtil {
	...
	public Set<String> getRoles(Connection con, String userName) throws Exception{
		...
		String sql="select * from t_user u,t_role r where u.roleId=r.id and u.name=?";
		PreparedStatement pstmt=con.prepareStatement(sql);
		pstmt.setString(1, userName);
		ResultSet rs=pstmt.executeQuery();
		while(rs.next()){
	...
	public Set<String> getPermissions(Connection con, String userName)throws Exception {
		...
		String sql="select * from t_user u,t_role r,t_permission p where u.roleId=r.id and p.roleId=r.id and u.name=?";
		PreparedStatement pstmt=con.prepareStatement(sql);
		pstmt.setString(1, userName);
		ResultSet rs=pstmt.executeQuery();
		while(rs.next()){
